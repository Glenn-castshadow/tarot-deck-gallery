  ...
  function attach(root) {
    ...
    function render() {
      ...
      try {
        ...
        output.innerHTML = `<article class="dh-reading"><div class="dh-dateline"><time datetime="${reading.day}">${esc(dateLabel)}</time><span>Today · ${esc(reading.sign)}</span></div><div class="dh-main"><span class="dh-sign" aria-hidden="true">${reading.glyph}</span><div><p class="dh-label">${esc(reading.sign)} · Today’s theme</p><h4>${esc(reading.title)}</h4><p>${esc(reading.overview)}</p></div></div><div class="dh-moon"><span aria-hidden="true">☾</span><div><strong>Moon in ${moon.sign} · ${reading.phase}</strong><p>${reading.illumination}% illuminated at the daily snapshot. ${esc(reading.phasePrompt)}</p></div></div><div class="dh-lenses">${reading.lenses.map(lens=>`<section><h5>${lens.title}</h5><p>${esc(lens.text)}</p></section>`).join('')}</div><div class="dh-action"><p class="dh-label">One small action</p><p>${esc(reading.action)}</p></div><blockquote>${esc(reading.question)}</blockquote></article>`;
      } catch (error) {
        renderedDay = '';
