...
`account-core.js` → `account.js` → `newsletter.js` → `birthday-insights.js` →
`birthplace-search.js` → `vendor/astronomy-engine/astronomy.browser.min.js` →
`natal-engine.js` → `chart-rooms.js` → `jyotish-engine.js` → `jyotish-text.js` →
`jyotish-chart.js` → `jyotish.js` → `birth-lore.js` → `birth-profile.js` →
`birth-form.js` → `natal-room.js` → `chinese-year.js` → `chinese-room.js` →
`mobile-sections.js`.
...
Nine elements in the whole site carry it today:
...
- `/charts/`'s `#celestial-extras` — `data-room="synastry composite davison bazi"`.
- `/eastern/`'s `#jyotish` — `data-room="jyotish"`.

`rooms.js`'s `PAGES` table lists `natal`, `solar-return`, `lunar-return`,
`progressed`, `synastry`, `composite`, `davison`, `horary`, `jyotish` and `bazi` as
kinds with a home page, and **every one of them now has a `Rooms.register()` caller**:
`tarot.js`, `divination.js`, `sky-calendar.js`, `numerology.js`, `chart-in-time.js`,
`horary.js`, `natal-room.js`, `celestial-extras.js` and `jyotish.js` (confirmed by
grepping every `.js` file for `Rooms.register`). A saved reading of any registered kind
now resolves correctly in `Rooms.openFromQuery`, and every kind in
`server/ishtar/readings/kinds.py` has a registered room — none are forward
declarations any more.
...
The C5a chart-saving project (2026-09-16) registered five: `natal` in
...
list above). The C5b chart-saving project (2026-09-16) registered the remaining five:
`synastry`, `composite`, `davison` and `bazi` in `celestial-extras.js`, with
`data-room` markup on `/charts/`'s `#celestial-extras`, and `jyotish` in `jyotish.js`,
with `data-room` markup on `/eastern/`'s `#jyotish`.
