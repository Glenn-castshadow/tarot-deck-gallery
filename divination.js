(() => {
  ...
  function showArt(kind,id,back=false) {
    const item=(kind==='geomancy'?D.figures:D[kind])[id]; artSelection={kind,id};
    artDialog.dataset.practice=kind;
    artDialog.innerHTML=`<header><h2>${back?modes[kind].name+' · Back':item.name}</h2><button type="button" data-dv-close aria-label="Close artwork">×</button></header><div class="dv-art-sides" role="group" aria-label="Card side"><button type="button" data-dv-side="front" aria-pressed="${!back}">Face</button>${kind!=='geomancy'?`<button type="button" data-dv-side="back" aria-pressed="${back}">Back</button>`:''}</div><div class="dv-art-large" role="img" aria-label="${back?modes[kind].name+' ornamental back':item.name+' illustrated face'}">${back?`<span class="dv-artwork"><img src="${artPath(kind,'back',true)}" alt="" width="${kind==='runes'?768:960}" height="${kind==='runes'?960:1536}"></span>`:visual(kind,item,true)}</div><p>${back?'The shared back for this collection.':item.keyword+' · '+item.prompt}</p>`;
  ...
  const states=Object.fromEntries(Object.keys(modes).map(key=>[key,{count:key==='lenormand'?3:1,question:'',reading:null,revealed:new Set(),study:0,manual:false,mothers:Array.from({length:4},()=>[2,2,2,2])}]));
  let mode='lenormand';
  function items() {return mode==='geomancy'?D.figures:D[mode];}
  ...
  function render() {
    ...
    root.style.setProperty('--dv-back',`url("${artPath(mode,mode==='geomancy'?0:'back')}")`);
    root.innerHTML=`<header class="dv-header"><div><p class="section-kicker">Cards &amp; divination</p><h2 id="divination-title">Four ways to listen closely.</h2><p>Choose a practice. Make a little room for what you notice.</p></div><span class="dv-seal" aria-hidden="true">✧</span></header>
      ...
      <details class="dv-library"><summary>Explore all ${items().length} ${mode==='runes'?'runes':mode==='geomancy'?'figures':'cards'}</summary><p>Choose a symbol to study its meaning without drawing a reading.</p><div class="dv-library-grid">${items().map((item,i)=>`<button type="button" data-dv-study="${i}" aria-pressed="${s.study===i}">${visual(mode,item)}<span>${item.name}</span></button>`).join('')}</div><article class="dv-study" tabindex="-1"></article></details></div>`;
    output(); study();
  }
  function study() {
  ...
  function output() {
    ...
    if(!r) {
      out.innerHTML=`<div class="dv-welcome"><div class="dv-back-fan" aria-hidden="true"><i>✧</i><i>${mode==='runes'?'ᚷ':mode==='geomancy'?'⠿':'✧'}</i><i>✧</i></div><p>${mode==='geomancy'?'A pattern is waiting to take shape.':'Take a breath before the first reveal.'}</p><small>${mode==='geomancy'?'Cast sixteen starting rows, then explore how the shield unfolds.':'Use the draw button above. Turn each card or rune when you are ready.'}</small>${mode!=='geomancy'?'<button type="button" class="dv-view-art" data-dv-art="0" data-dv-back>Explore the artwork</button>':''}</div>`; return;
    }
  ...
  root.addEventListener('change',event=>{
    ...
    if(event.target.id==='dv-manual') states.geomancy.manual=event.target.checked;
  });
  root.addEventListener('click',event=>{
    ...
    if(button.hasAttribute('data-dv-draw')) {
      try {
        s.reading=mode==='geomancy'?{chart:s.manual?E.shield(s.mothers):E.cast(),selected:14,question:s.question}:{ids:E.draw(items().length,s.count),question:s.question};
        s.revealed.clear();output();button.innerHTML='Begin a new reading <span aria-hidden="true">↗</span>';
        root.querySelector('.dv-status').textContent=mode==='geomancy'?'Your shield is cast. Select a figure to read it.':`${s.count} ${mode==='runes'?'runes':'cards'} laid face down. Reveal them below.`;
    ...
    if(!s.reading || mode==='geomancy') return;
    let index;
