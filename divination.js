(() => {
  ...
  function study() {
    ...
    if(mode==='iching') {
      ...
      const hasLines=typeof IChingLines!=='undefined'&&IChingLines.has(item.number);
      const linesSection=hasLines?`<section class="dv-iching-lines" aria-label="The six lines"><ol role="list">${D.linePositions.map((p,i)=>`<li><strong>Line ${i+1} · ${p.title}</strong><p>${IChingLines.forLine(item.number,i)}</p></li>`).join('')}</ol></section>`:'';
      root.querySelector('.dv-study').innerHTML=`<p class="dv-kicker">${item.number} · ${item.keyword}</p><h4><span lang="zh">${item.character}</span> ${item.name}</h4><p class="dv-position-note">${item.gloss}</p><p>${upper.name} ${upper.character} above, ${lower.name} ${lower.character} below.</p><p>${item.meaning}</p>${linesSection}<blockquote>${item.prompt}</blockquote><button type="button" class="dv-view-art" data-dv-art="${item.id}">View the figure</button>`;
      return;
    }
  ...
  function hexagramOutput(out,r) {
    ...
    const changing=read.changing.map(i=>{const p=D.linePositions[i];const text=typeof IChingLines!=='undefined'&&IChingLines.has(h.number)?IChingLines.forLine(h.number,i):p.text;return `<article class="dv-line-reading"><p class="dv-kicker">Line ${i+1} · ${r.lines[i]===9?'old yang becoming yin':'old yin becoming yang'} · ${p.title}</p><p>${text}</p></article>`;}).join('');
