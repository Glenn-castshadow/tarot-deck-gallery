(() => {
  ...
  const labels = {lenormand: 'Lenormand', 'grand-tableau': 'Grand Tableau', oracle: 'Ishtar Reflection Oracle', runes: 'Runes', geomancy: 'Geomancy', 'geomancy-houses': 'Geomantic house chart', iching: 'I Ching'};
  ...
  root.addEventListener('click',event=>{
    ...
    if(button.hasAttribute('data-dv-draw')) {
      try {
        ...
        root.querySelector('.dv-status').textContent=mode==='geomancy'?(s.view==='houses'?'Your shield is cast and shown as a house chart. Select a house to read it.':'Your shield is cast. Select a figure to read it.'):mode==='iching'?'Six lines cast. Read the hexagram below.':s.reading.ids.length===36?'All 36 cards laid out. Select any position to read it.':`${s.count} ${mode==='runes'?'runes':'cards'} laid face down. Reveal them below.`;
